...
public class BootstrapData {
    ...
    public static List<Questao> buildQuestao(Disciplina disciplina) {
        return List.of(
                new Questao("Considere a seguinte tabela de um banco de dados relacional: Item (Código, Tipo, Nome, Quantidade) O comando SQL para obter o tipo e o valor médio da quantidade, por tipo de itens, apenas para valores médios superiores a 500, é:", disciplina),
                new Questao("Considerando um banco de dados relacional, sobre o conceito de Chave estrangeira, assinale a alternativa correta.", disciplina),
                new Questao("Não pode ter valor nulo em uma tabela do banco de dados um campo: ", disciplina),
                new Questao("Em um banco de dados, uma das principais restrições é a que define que, em uma relação entre duas tabelas, a chave estrangeira que aparece em uma delas deve existir como chave primária na outra, de forma que só será aceito um valor no campo chave estrangeira se esse valor existir no campo chave primária. Tal restrição é conhecida como", disciplina),
                new Questao("Em modelagem de dados a cardinalidade é um dos princípios fundamentais sobre relacionamento de um banco de dados relacional. No modelo relacional, podemos ter os seguintes níveis clássicos de relacionamento. Sobre eles, assinale a alternativa incorreta.", disciplina),
                new Questao("Um banco de dados baseado em tabelas, entidades e atributos é do tipo:", disciplina));
    }
    public static List<Alternativa> buildAlternativa(Questao questao) {
        return List.of(
                new Alternativa("SELECT Tipo, Quantidade-Média FROM Item HAVING Quantidade-Média > 500;", false, questao),
                new Alternativa("SELECT Tipo, LIMIT (Quantidade) > 500 FROM Item GROUP BY Tipo", false, questao),
                new Alternativa("SELECT Tipo, AVG (Quantidade) FROM Item GROUP BY Tipo HAVING AVG (Quantidade) > 500;", true, questao),
                new Alternativa("SELECT Tipo, MED (Quantidade) FROM Item GROUP BY Tipo WHERE MED (Quantidade) > 500;", false, questao),
                new Alternativa("SELECT Tipo, MAX - MIN (Quantidade) FROM Item GROUP BY Tipo FOR MAX – MIN (Quantidade) > 500;", false, questao)
        );
    }
    /*
